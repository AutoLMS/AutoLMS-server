...
class CourseService(BaseService):
    ...
    async def get_courses(
        ...
        if not force_refresh:
            existing_courses = await self.course_repository.get_by_user_id(db, user_id)
            if existing_courses:
        ...
        courses_data = self.course_parser.parse_courses(html)
        if not courses_data:
        ...
        for course_data in courses_data:
            try:
                ...
                if course_data['id'] in existing_course_ids:
                    ...
                    # 새 강의 추가
                    updated_course = await self.course_repository.create(db, course_dict)
    ...
    async def get_course(self, user_id: str, course_id: str, db: AsyncSession) -> Optional[Course]:
        ...
        # 강의 정보 조회
        course = await self.course_repository.get_by_id(db, course_id)
    ...
    async def get_course_menus(self, user_id: str, course_id: str) -> Dict[str, Dict[str, Any]]:
        ...
        course_menus = self.course_parser.parse_course_menus(response.text)
